...
`versions.lock` her vendor deposunun commit'ini sabitler. Kurulum iki ayrı, bağımsız adımdır: **repo-local** (`install.sh`) ve **global** (`global-install.sh`). İkisi de `ai_code_stack/filesystem.py`'deki aynı güvenlik kuralına uyar: `atomic_write_text`, hedefte `GENERATED BY AI CODE STACK` işareti taşımayan (yani elle yazılmış) bir dosya varsa üzerine yazmayı reddeder — bu güvenlik testlerle doğrulanmıştır.

### Kolay yol: çift tıkla

- **macOS:** `AI Code Stack Kur.command` dosyasına çift tıkla. Terminal açılır, önce repo-local kurulumu, sonra onay isteyip global kurulumu çalıştırır.
- **Windows:** `AI Code Stack Kur.bat` dosyasına çift tıkla. Aynı akışı PowerShell üzerinden çalıştırır.
...
`global-install.sh --apply` (→ `ai_code_stack.global_install.global_install`) — repo dışına, gerçek sistem konumlarına yazan **tek** adım, bu yüzden varsayılan `--apply` olmadan yalnızca dry-run rapor verir:
